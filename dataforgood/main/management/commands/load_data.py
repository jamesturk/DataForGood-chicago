...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        self.load_tract_zip_codes_data(os.path.join(base_dir, 'TractZipCodes.csv'))

        # Process other CSV files
        for filename in os.listdir(base_dir):
            if filename.endswith('.csv') and filename not in ['CensusTracts.csv', 'TractZipCodes.csv']:
    ...
    def load_model_data(self, base_dir, filename):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"Successfully loaded data into {model_name}"))
        except LookupError:
            self.stdout.write(self.style.ERROR(f"No model found for {model_name}"))
        ...
        with open(file_path, newline='') as file:
            ...
            for record in reader:
                try:
                    ...
                    self.stdout.write(self.style.ERROR(f"Error saving record: {str(e)}"))
                    continue
            model.objects.bulk_create(instances, ignore_conflicts=True)
            self.stdout.write(self.style.SUCCESS(f"Successfully loaded data into CensusTracts"))
        ...
        with open(file_path, newline='') as file:
            ...
            for record in reader:
                try:
                    ...
                    tract = TractModel.objects.get(tract_id=int(record['tract_id']))
                    # Create an instance of TractZipCode with the correct ForeignKey association
                    zip_code_instance = ZipCodeModel(tract=tract, zip_code=int(record['zip_code']))
                    instances.append(zip_code_instance)
                except TractModel.DoesNotExist:
                    self.stdout.write(self.style.ERROR(f"Census tract not found for ID {record['tract_id']}"))
                    continue
                except IntegrityError as e:
                    self.stdout.write(self.style.ERROR(f"Error saving record: {str(e)}"))
                    continue
                except ValueError as e:
                    self.stdout.write(self.style.ERROR(f"Data conversion error: {str(e)}"))
                    continue
            # Bulk create all the gathered instances to the database
            ZipCodeModel.objects.bulk_create(instances, ignore_conflicts=True)
            self.stdout.write(self.style.SUCCESS(f"Successfully loaded data into TractZipCodes"))
        ...
        with open(file_path, newline='') as file:
            ...
            for record in reader:
                try:
                    ...
                    self.stdout.write(self.style.ERROR(f"Error parsing record: {str(e)}"))
                    continue
            model.objects.bulk_create(instances, ignore_conflicts=True)
            self.stdout.write(self.style.SUCCESS(f"Successfully loaded data into {model.__name__}"))