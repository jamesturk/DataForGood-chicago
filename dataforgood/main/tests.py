...
from .utils import create_table, create_subgroup_table_rows, \
    MAIN_MODEL_MAPPING, SUB_MODEL_MAPPING
from .models import TractZipCode, CensusTracts

INDICATORS_LIST = list(MAIN_MODEL_MAPPING.keys())

TRACT_LIST = [tup[0] for tup in list(
    TractZipCode.objects.values_list("tract_id").distinct())]

ZIPCODE_LIST = [tup[0] for tup in list(
    TractZipCode.objects.values_list("zip_code").distinct())]

COMMUNITY_LST = [tup[0] for tup in list(
    CensusTracts.objects.values_list("community").distinct())]

PERIOD_LIST = ["2013-2017", "2014-2018", "2015-2019", "2016-2020", "2017-2021",
               "2018-2022"]
...
class CreateMainTableTests(TestCase):
    ...
    def test_city_table_check_for_none(self):
        ...
        for indicator in INDICATORS_LIST:
            rows = create_table(geographic_level, geographic_unit, 
                                indicator, periods)['rows']
            for row in rows:
